...
class PostprocessYOLO(object):
    ...
    def _process_feats(self, output_reshaped, mask):
        ...
        #print('yolo1',grid_h,grid_w)
        #print('yolo1', output_reshaped.shape)
...
class TrtYOLOv3_3(object):
    """TrtYOLOv3 class encapsulates things needed to run TRT YOLOv3."""

    def _load_engine(self):
        TRTbin_2 = '/home/cv/Downloads/tensorrt_demos-master/tensorrt_demos-master/yolov3_onnx/ocr-2.trt'
        #print(TRTbin_2)
        
        with open(TRTbin_2, 'rb') as e, trt.Runtime(self.trt_logger) as runtime:
            return runtime.deserialize_cuda_engine(e.read())

    def _create_context(self):
        return self.engine.create_execution_context()

    def __init__(self, model, input_shape=(64, 352)):
        """Initialize TensorRT plugins, engine and conetxt."""
        self.model = model
        self.input_shape = input_shape
        h, w = input_shape
        if 'tiny' in model:
            self.output_shapes = [(1, 66, h // 32, w // 32),
                                  (1, 66, h // 16, w // 16)]
        else:
            self.output_shapes = [(1, 21, h // 32, w // 32),
                                  (1, 21, h // 16, w // 16),
                                  (1, 21, h //  8, w //  8)]
        if 'tiny' in model:
            postprocessor_args = {
    ...
    def detect(self, img, conf_th=0.4):
